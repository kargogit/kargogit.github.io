...
const sections = [
  ...
  { id: 'employment', title: 'Employment History', content: '' },
  { id: 'skills', title: 'Skills', content: '' },
  { id: 'projects', title: 'Projects', content: '' },
  { id: 'contact', title: 'Contact', content: 'Feel free to reach out to me via email or follow me on social media!' }
];
...
const skills = [
  ...
  {
    category: 'CI/CD',
    items: ['Docker', 'Kubernetes', 'AWS', 'Maven']
  },
  {
    category: 'Libraries & Frameworks',
    items: ['Django', 'Spring', 'React', 'Tailwind', 'RabbitMQ', 'jBPM', 'Numpy + Pandas + Scikit-Learn']
  },
  {
    category: 'Databases',
    items: ['SQL', 'MongoDB']
  },
  {
    ...
    items: ['Windows', 'Linux', 'BSD']
  },
  {
    category: 'Others',
    items: ['Matplotlib', 'PowerBI', 'Tableau', 'Jupyter Notebook']
  }
];
...
const education = [
  ...
  },
  {
    ...
    degree: 'BE in Computer Science & Engg',
    duration: '2017 - 2021',
    ...
    <nav className="container mx-auto flex justify-between items-center px-6">
      ...
      <ul className="flex space-x-4">
        {sections.map(section => (
          <li key={section.id}>
            <a href={`#${section.id}`} className="hover:text-blue-500 transition duration-300">{section.title}</a>
          </li>
        ))}
      </ul>
    </nav>
...
const SkillsSection = () => (
  <Section id="skills" title="Skills">
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      {skills.map(skill => (
        <div key={skill.category} className="bg-white shadow-lg rounded-lg p-6 transform hover:scale-105 transition duration-300">
    ...
    <div className="space-y-6">
      {employmentHistory.map((job, index) => (
        <div key={index} className="bg-white shadow-lg rounded-lg p-6">
          ...
          <p className="text-l font-semibold">{job.position}</p>
          <h4 className="text-sm text-gray-600">{job.duration}</h4>
          <ul>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {education.map((edu, index) => (
        <div key={index} className="bg-white shadow-lg rounded-lg p-6 transform hover:scale-105 transition duration-300">
          ...
          <p className="text-l font-semibold">{edu.degree}</p>
      ...
      <main className="container mx-auto px-6 py-12">
        <Section id="about" title="About Me">
          <p className="text-lg leading-relaxed max-w-2xl mx-auto">
            Versatile Software Developer with a track record at multinational firms, committed to delivering innovative solutions leveraging diverse tech stacks.
          </p>
        </Section>
        <SkillsSection />
        <EmploymentSection />
